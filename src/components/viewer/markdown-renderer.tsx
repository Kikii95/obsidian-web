      ...
      <ReactMarkdown
        ...
        components={{
          ...
          input: ({ type, checked, disabled, ...props }) => {
            if (type === "checkbox") {
              // This is a task list checkbox from remark-gfm
              // We need to find the task text from sibling nodes (handled in parent)
              // For now, render as interactive if allowed
              return (
                <input
                  type="checkbox"
                  checked={checked}
          ...
          li: ({ children, className, node, ...props }) => {
            ...
            if (isTaskItem && canToggleCheckbox && onCheckboxToggle) {
              ...
              return (
                <li
                  className={cn("flex items-start gap-0 list-none", className)}
